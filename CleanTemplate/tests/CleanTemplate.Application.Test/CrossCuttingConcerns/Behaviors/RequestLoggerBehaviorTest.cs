...
namespace CleanTemplate.Application.Test.CrossCuttingConcerns.Behaviors
{
    public class RequestLoggerBehaviorTest
    {
        ...
        public async Task Handle_Success()
        {
            ...
            Assert.Equal("{ status: {@Status}, response: {@Request}, userId {@UserId}, user: {@User}, data: {@Data} }", responseMsg);
        ...
        {
            var logger = A.Fake<ILoggerAdapter<RequestLoggerBehavior<SampleRequestCustomLogging, SampleResponseCustomLogging>>>();
            var userService = new FakeUserService();
            var sut = new RequestLoggerBehavior<SampleRequestCustomLogging, SampleResponseCustomLogging>(logger, userService);
        ...
        public async Task Handle_WithApplicationException_LogsOnlyInformation()
        {
            ...
            catch (AppException e)
            {
                ...
                var (responseMsg, responseArgs) = capturedCalls[1];
                Assert.Equal("{ status: {@Status}, request: {@Request}, userId: {@UserId}, user: {@User}, error: @{Failure} }", responseMsg);
        ...
        public async Task Handle_UnExpectedException_LogsError()
        {
            ...
            catch (Exception e)
            {
                ...
                var (ex, responseMsg, responseArgs) = capturedErrorCalls[0];
                Assert.Equal("Exception Message", e.Message);
                Assert.Equal("{ status: {@Status}, request: {@Request}, userId: {@UserId}, user: {@User} }", responseMsg);
